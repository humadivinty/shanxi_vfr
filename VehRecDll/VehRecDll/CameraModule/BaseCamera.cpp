...
void BaseCamera::ReadConfig()
{
	...
	iTemp = 0;
	Tool_ReadIntValueFromConfigFile(INI_FILE_NAME, "Video", "SaveEnable", iTemp);
	m_bSaveVideoEnable = (iTemp == 0) ? false : true;
}
...
std::string BaseCamera::createVideoFileName(const char* timeString)
{
	char chImgDir[256] = { 0 };
	GetImageDir(chImgDir, sizeof(chImgDir));
	std::string strPlateTime(timeString);
